...
function openDetailLevel(elem) {
  ...
  switch (elem.currentTarget.id){
    case 'slb2':
      dModal.querySelector('.detail-modal-body').innerHTML = getSymptomTemplate(JSON_contents[type].symptoms);
      break;
    case 'slb3':
      dModal.querySelector('.detail-modal-body').innerHTML = getWhoIsAtRiskTemplate(JSON_contents[type].whois);
      break;
    case 'slb4':
      dModal.querySelector('.detail-modal-body').innerHTML = getComplicationsTemplate(JSON_contents[type].complications);
      break;
